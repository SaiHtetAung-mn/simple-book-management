    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if(flag) {
                ...
                reject(new BassError(
                    404, 
                    true, 
                    `Update failed! No book with id ${id} exits`
                ));
            }
        }
        catch(err) {
...
function deleteBookById(id) {
    return new Promise(async (resolve, reject) => {
        ...
        if(book) {
            ...
            reject(new BaseError(404, true, `No book with id: ${id} exits`));
        }
    })
}
    ...
    try {
        fs.stat(filePath, (err, stats) => {
            if(err) {
                ...
                throw new BaseError(404, false, `File path: ${filePath} exits`);
            }
        });
